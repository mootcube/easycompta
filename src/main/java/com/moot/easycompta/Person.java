...
public class Person implements Actor{
    ...
    {
        if(p==null)warn("Person.addAccount - PersonalAccount is null. "+this);
        if(accounts.containsKey(p.getId()))throw new Exception("an other account with the same id is already registered with this person");
        if(!p.getOwner().equals(this))throw new Exception("different owner");
        accounts.put(p.getId(),p);
    }
    //peut être changer personalaccount en classe interne et faire des accès avec les strings comme id.
